...
3. OpenClaw Gateway loads `openclaw.json` from `openclaw-config` (generated from `agents/agents.yaml`) and mounts agent prompts directly from the shared `agents-config` volume.
...
- **OpenClaw prompts source of truth**: `agents/<AgentName>/AGENTS.md` (mounted via `agents-config` PVC)
